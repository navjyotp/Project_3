...
def ros_inputs(Theta_i,u_l,u_r):
    r = 0.066
    L = 0.178
    # X_n = X_i
    # Y_n = Y_i
    Theta_n = 3.14*Theta_i/180
    Theta_dot = (r/L)*(u_r-u_l)
    Theta_n = Theta_n + Theta_dot
    x_dot = (r/2)*(u_l+u_r)*math.cos(Theta_dot)
    y_dot = (r/2)*(u_l+u_r)*math.sin(Theta_dot)
    vel = math.sqrt(x_dot**2 + y_dot**2)
    Theta_n = 180*Theta_n/3.14
    return vel, Theta_dot, Theta_n
...
roslaunch.configu_re_logging(uuid)
x_pos = 'x_pos:="' + str(float(initial_x)/100.0 - 5.0) + '"'
y_pos = 'y_pos:="' + str(float(initial_y)/100.0 - 5.0) + '"'
yaw = 'yaw:="' + str(math.radians(intial_angle)) + '"'
cli_args = ['project3','turlebot_launch.launch',x_pos,y_pos,yaw]
...
turtlebot_model = rospy.get_param("model", "waffle")
...
for i in range(len(actions)):
    ...
    while not rospy.is_shutdown():
        rpm_L = actions[i][0]
        rpm_R = actions[i][1]
        u_l = (2*3.14*r*rpm_L/60)
        if u_l > 0.22:
            u_l = 0.22
        u_r = (2*3.14*r*rpm_R/60)
        if u_r > 0.22:
            u_r = 0.22
        while t < 1:
            t = t + dt
            vel, theta_dot, Theta_n = ros_inputs(Theta_n,u_l,u_r)
            msg.linear.x = vel*10
            msg.angu_lar.z = theta_dot * 10
